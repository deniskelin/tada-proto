// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.27.1
// 	protoc        v3.19.3
// source: tada/gateway/admin/meeting/calendar_service.proto

package meetingGatewayPb

import (
	meeting "github.com/deniskelin/tada-proto/tada/api/meeting"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/known/timestamppb"
	wrapperspb "google.golang.org/protobuf/types/known/wrapperspb"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_tada_gateway_admin_meeting_calendar_service_proto protoreflect.FileDescriptor

var file_tada_gateway_admin_meeting_calendar_service_proto_rawDesc = []byte{
	,
}

var file_tada_gateway_admin_meeting_calendar_service_proto_goTypes = []interface{}{
	(*meeting.GetMeetingsRequest)(nil),             // 0: tada.api.meeting.GetMeetingsRequest
	(*wrapperspb.Int64Value)(nil),                  // 1: google.protobuf.Int64Value
	(*wrapperspb.StringValue)(nil),                 // 2: google.protobuf.StringValue
	(*meeting.CreateMeetingRequest)(nil),           // 3: tada.api.meeting.CreateMeetingRequest
	(*meeting.UpdateMeetingRequest)(nil),           // 4: tada.api.meeting.UpdateMeetingRequest
	(*meeting.GetMeetingMembersRequest)(nil),       // 5: tada.api.meeting.GetMeetingMembersRequest
	(*meeting.AddMemberInMeetingRequest)(nil),      // 6: tada.api.meeting.AddMemberInMeetingRequest
	(*meeting.UpdateMemberInMeetingRequest)(nil),   // 7: tada.api.meeting.UpdateMemberInMeetingRequest
	(*meeting.DeleteMemberFromMeetingRequest)(nil), // 8: tada.api.meeting.DeleteMemberFromMeetingRequest
	(*meeting.GetMeetingsResponse)(nil),            // 9: tada.api.meeting.GetMeetingsResponse
	(*meeting.Meeting)(nil),                        // 10: tada.api.meeting.Meeting
	(*meeting.SuccessResponse)(nil),                // 11: tada.api.meeting.SuccessResponse
	(*meeting.GetMeetingMembersResponse)(nil),      // 12: tada.api.meeting.GetMeetingMembersResponse
	(*meeting.Member)(nil),                         // 13: tada.api.meeting.Member
}
var file_tada_gateway_admin_meeting_calendar_service_proto_depIdxs = []int32{
	0,  // 0: tada.gateway.admin.meeting.MeetingGateway.GetMeetings:input_type -> tada.api.meeting.GetMeetingsRequest
	1,  // 1: tada.gateway.admin.meeting.MeetingGateway.GetMeetingById:input_type -> google.protobuf.Int64Value
	2,  // 2: tada.gateway.admin.meeting.MeetingGateway.GetMeetingByChatUUID:input_type -> google.protobuf.StringValue
	3,  // 3: tada.gateway.admin.meeting.MeetingGateway.CreateMeeting:input_type -> tada.api.meeting.CreateMeetingRequest
	4,  // 4: tada.gateway.admin.meeting.MeetingGateway.UpdateMeeting:input_type -> tada.api.meeting.UpdateMeetingRequest
	1,  // 5: tada.gateway.admin.meeting.MeetingGateway.DeleteMeeting:input_type -> google.protobuf.Int64Value
	5,  // 6: tada.gateway.admin.meeting.MeetingGateway.GetMeetingMembers:input_type -> tada.api.meeting.GetMeetingMembersRequest
	6,  // 7: tada.gateway.admin.meeting.MeetingGateway.AddMemberInMeeting:input_type -> tada.api.meeting.AddMemberInMeetingRequest
	7,  // 8: tada.gateway.admin.meeting.MeetingGateway.UpdateMemberInMeeting:input_type -> tada.api.meeting.UpdateMemberInMeetingRequest
	8,  // 9: tada.gateway.admin.meeting.MeetingGateway.DeleteMemberFromMeeting:input_type -> tada.api.meeting.DeleteMemberFromMeetingRequest
	9,  // 10: tada.gateway.admin.meeting.MeetingGateway.GetMeetings:output_type -> tada.api.meeting.GetMeetingsResponse
	10, // 11: tada.gateway.admin.meeting.MeetingGateway.GetMeetingById:output_type -> tada.api.meeting.Meeting
	10, // 12: tada.gateway.admin.meeting.MeetingGateway.GetMeetingByChatUUID:output_type -> tada.api.meeting.Meeting
	10, // 13: tada.gateway.admin.meeting.MeetingGateway.CreateMeeting:output_type -> tada.api.meeting.Meeting
	10, // 14: tada.gateway.admin.meeting.MeetingGateway.UpdateMeeting:output_type -> tada.api.meeting.Meeting
	11, // 15: tada.gateway.admin.meeting.MeetingGateway.DeleteMeeting:output_type -> tada.api.meeting.SuccessResponse
	12, // 16: tada.gateway.admin.meeting.MeetingGateway.GetMeetingMembers:output_type -> tada.api.meeting.GetMeetingMembersResponse
	13, // 17: tada.gateway.admin.meeting.MeetingGateway.AddMemberInMeeting:output_type -> tada.api.meeting.Member
	13, // 18: tada.gateway.admin.meeting.MeetingGateway.UpdateMemberInMeeting:output_type -> tada.api.meeting.Member
	11, // 19: tada.gateway.admin.meeting.MeetingGateway.DeleteMemberFromMeeting:output_type -> tada.api.meeting.SuccessResponse
	10, // [10:20] is the sub-list for method output_type
	0,  // [0:10] is the sub-list for method input_type
	0,  // [0:0] is the sub-list for extension type_name
	0,  // [0:0] is the sub-list for extension extendee
	0,  // [0:0] is the sub-list for field type_name
}

func init() { file_tada_gateway_admin_meeting_calendar_service_proto_init() }
func file_tada_gateway_admin_meeting_calendar_service_proto_init() {
	if File_tada_gateway_admin_meeting_calendar_service_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_tada_gateway_admin_meeting_calendar_service_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_tada_gateway_admin_meeting_calendar_service_proto_goTypes,
		DependencyIndexes: file_tada_gateway_admin_meeting_calendar_service_proto_depIdxs,
	}.Build()
	File_tada_gateway_admin_meeting_calendar_service_proto = out.File
	file_tada_gateway_admin_meeting_calendar_service_proto_rawDesc = nil
	file_tada_gateway_admin_meeting_calendar_service_proto_goTypes = nil
	file_tada_gateway_admin_meeting_calendar_service_proto_depIdxs = nil
}
